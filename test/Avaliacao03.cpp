...
void init() {
  ...
  loadTGA("Avaliacao03/f1.tga");

  loadObj("Avaliacao03/f1.obj", body);
  loadObj("Avaliacao03/tyre_front.obj", tyre_front);
  loadObj("Avaliacao03/tyre_back.obj", tyre_back);
}
...
void specialKeys(int key, int x, int y) {
  switch (key) {
    ...
    case 'z':
      steer += 2;
      steer = (steer > 10) ? 10 : steer;
      break;
    case 'x':
      steer -= 2;
      steer = (steer < -20) ? -20 : steer;
      break;
    case 32:
      angleX = 0;
      angleY = 0;
...
void drive() {
  spin += 20;
  steer += (turn == true) ? 0.5 : -0.5;
  if (steer > 20) turn = false;

  if (steer < -20) turn = true;
